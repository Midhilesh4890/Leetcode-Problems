...
train_cols = ['T' + str(i) for i in range(1, cols + 1)]
train = pd.DataFrame(np.random.rand(100, cols), columns=train_cols)

rest_cols = ['R' + str(i) for i in range(1, cols + 1)]
rest = pd.DataFrame(np.random.rand(10000, cols), columns=rest_cols)
...
if __name__ == '__main__':
    ...
    with Pool(processes=4) as pool:  # You can adjust the number of processes as needed
        ...
        with tqdm(total=len(RANGES)) as pbar:
            ...
            pbar.update(len(RANGES))  # Update the progress bar for all tasks
